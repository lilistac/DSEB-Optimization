            ...
            with option2:
                ...
                if option_un_opti1 == 'Gradient Descent Method':
                    ...
                    if  option_un_opti_1 == "Exercise":
                        ...
                        if max_min1 == "Minimize":
                            ...
                            option = 2
                        try: 
                            obj_function, initial_val, max_iter = input_uc2()
                ...
                if option_un_opti1 == 'Newton Raphson Method':
                    ...
                    if option_un_opti_new == "Theory":
                        ...
                        st.write("Newton's method is a general approach for solving systems of non-linear equations.Newton's method can")
                        st.write("conceptually be seen as a steepest descent method, and we will show how it can be applied for convex optimization.")
                        Theory_Newton ()
                    if option_un_opti_new == "Exercise":
                        ...
                        if max_min2 == "Minimize":
                            ...
                            option = 2
                        try:
                            obj_function, initial_val, max_iter = input_uc1(2)
    ...
    with option3:
        ...
        if option_con_opti =="Linear Programming Problem":
            ...
            if option_con_opti_sim == "Theory":
                ...
                st.write("of an optimization problem. Simplex tableau is used to perform row operations on the\
            ...
            if option_con_opti_sim == "Exercise":
                ...
                st.latex(r'\text{Example:}')
                st.latex(r'''\text{Minimize }f(x) = 2x_1 + 5x_2 + 4x_3 + x_4 - 5x_5
            ...
            with option4:
                ...
                if option_con_opti1 == "Under equality constraints":
                    ...
                    if option_con_opti_non_lag == "Theory":
                        ...
                        st.write("in conjunction with linear approximations to the constraints and sometimes \
                    ...
                    if option_con_opti_non_lag == "Exercise":
                        ...
                        try:
                            objFunction = st.sidebar.text_input("Enter object function (f) ")
                            constraints = []
                            m = st.sidebar.text_input('Number of equality constrained: ')
                            for i in range(int(m)):
                                constraints.append(st.sidebar.text_input(f'Constraint g{i+1} = '))
                ...
                if option_con_opti1 == "Under equality and unequality constraints":
                    ...
                    if option_con_opti_non_2 == "Theory":
                        ...
                        st.write("in nonlinear programming to be optimal, provided that some regularity conditions are \
                                    satisfied.")
                    ...
                    if option_con_opti_non_2 == "Exercise":
                        ...
                        try:
                            ...
                            m = st.sidebar.text_input('Number of equality constrained: ')
                            for i in range(int(m)):
                                eqConstraints.append(st.sidebar.text_input(f'Equality constraint h{i+1} = '))

                            with st.sidebar:
                                st.write("Form of inequality constraints: ")
                                st.latex("g_i <= 0")
                            p = st.sidebar.text_input('Number of inequality constrained: ')
                            for i in range(int(p)):
                                constraints = st.sidebar.text_input(f'Inequality constraint g{i+1} = ')
                                ineqConstraints.append(constraints)
...
if add_selectbox =="Application of Optimization":
    st.image("image//thanks.png")
    with option4:
        st.title(r'''Welcome to Travelling Salesman Problem ♡''')
        ...
        st.write("and locations that must be visited. In the problem statement, the points are the cities a salesperson might visit.")
